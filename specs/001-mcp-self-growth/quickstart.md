...
- SSH疎通: `ssh ubuntu@YOUR_SERVER_IP docker ps` が成功すること
- Prometheusが稼働: `http://YOUR_SERVER_IP:9090` にアクセスできること
...
# 疎通確認
ssh -i ~/.ssh/monitoring_lab_key ubuntu@YOUR_SERVER_IP echo "OK"
```

### Prometheus接続エラー
```bash
# エンドポイント確認
curl http://YOUR_SERVER_IP:9090/api/v1/query?query=up
```
