...
VictoriaMetrics シングルノードを Docker コンテナとして `YOUR_SERVER_IP` にデプロイし、Prometheus の `remote_write` で全メトリクスを自動転送する。保持期間 12ヶ月（FR-002）、Prometheus 互換 API による既存クエリの再利用（FR-003）、Grafana への独立データソース追加（FR-005）を実現する。
...
**Target Platform**: Docker Engine on Ubuntu 22.04 (YOUR_SERVER_IP)  
...
**検証**:
```bash
curl -sf http://YOUR_SERVER_IP:8428/health  # → "OK"
```
...
**検証**:
```bash
curl -s "http://YOUR_SERVER_IP:8428/api/v1/query?query=up"  # → status: success
```
