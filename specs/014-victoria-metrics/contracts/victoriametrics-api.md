...
GET http://10.0.0.220:8428/health
→ 200 OK: "OK"
```
...
curl -s "http://10.0.0.220:8428/api/v1/query?query=up" | python3 -m json.tool

# 長期クエリ（例: 90日前からの Prometheus 稼働状況）
curl -s "http://10.0.0.220:8428/api/v1/query_range?query=up&start=$(date -d '90 days ago' +%s)&end=$(date +%s)&step=3600"
