...
1. **Given** Vault dev モードが稼働中（`http://YOUR_SERVER_IP:8200`）、**When** オペレーターが Terragrunt apply を実行する、**Then** `secret/monitoring-lab/alertmanager` パスに Webhook URL が格納される
...
- Terragrunt コンテナから Vault（YOUR_SERVER_IP:8200）へのネットワーク疎通が取れない場合 → `plan` 時にエラーで検出できること
...
- `sync-config.sh` を実行するホストから Vault API（`http://YOUR_SERVER_IP:8200`）にアクセスできる
