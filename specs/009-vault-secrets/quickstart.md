...
- [ ] WSL2 から Vault API に接続可能（`curl http://YOUR_SERVER_IP:8200/v1/sys/health`）
...
curl -s \
  ...
  http://YOUR_SERVER_IP:8200/v1/secret/data/monitoring-lab/alertmanager \
  | jq '.data.data'
...
```bash
# amtool で設定検証
ssh ubuntu@YOUR_SERVER_IP "docker exec monitoring-lab-alertmanager \
...
```bash
# Vault の稼働確認
curl -s http://YOUR_SERVER_IP:8200/v1/sys/health | jq '.initialized'
# true であれば稼働中

# シークレットの存在確認
curl -s -H "X-Vault-Token: root" \
  http://YOUR_SERVER_IP:8200/v1/secret/metadata/monitoring-lab/alertmanager \
  | jq '.data'
```
...
```bash
# vault コンテナ確認
wsl -d Ubuntu-24.04 -e bash -c "ssh ubuntu@YOUR_SERVER_IP 'docker ps | grep vault'"
