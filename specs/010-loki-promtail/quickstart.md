...
scp config/loki/loki.yml ubuntu@10.0.0.220:/home/ubuntu/monitoring-lab/loki/loki.yml
scp config/promtail/promtail.yml ubuntu@10.0.0.220:/home/ubuntu/monitoring-lab/promtail/promtail.yml
...
# ブラウザ: https://app.terraform.io → Organizations → k1981-learning-lab
...
```bash
# リモートサーバーから
ssh ubuntu@10.0.0.220 'curl -s http://localhost:3100/ready'
# 期待値: "ready"

# コンテナ状態確認
ssh ubuntu@10.0.0.220 'docker ps | grep loki'
...
```bash
# コンテナ状態確認
ssh ubuntu@10.0.0.220 'docker ps | grep promtail'
# 期待値: monitoring-lab-promtail が Up

# Promtail のログ確認（Docker Socket 接続成功を確認）
ssh ubuntu@10.0.0.220 'docker logs monitoring-lab-promtail 2>&1 | head -20'
...
```bash
# Docker Socket 権限確認
ssh ubuntu@10.0.0.220 'ls -la /var/run/docker.sock'
# 期待値: srw-rw---- ... docker グループ

# Promtail コンテナ内から確認
ssh ubuntu@10.0.0.220 'docker exec monitoring-lab-promtail ls /var/run/docker.sock'
```
...
```bash
# Promtail のエラーログを確認
ssh ubuntu@10.0.0.220 'docker logs monitoring-lab-promtail 2>&1 | grep -i error'

# Loki への疎通確認（Promtailコンテナ内から）
ssh ubuntu@10.0.0.220 'docker exec monitoring-lab-promtail wget -qO- http://loki:3100/ready'
```
...
ssh ubuntu@10.0.0.220 'docker logs monitoring-lab-grafana 2>&1 | grep -i loki'
```
