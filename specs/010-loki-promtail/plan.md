...
Loki（ログ集約）と Promtail（ログ収集エージェント）を既存の docker_container Terragrunt モジュールでリモートDocker環境（YOUR_SERVER_IP）にデプロイする。Promtail は Docker Socket 経由で全監視基盤コンテナのログを収集し、Grafana から LogQL で検索・閲覧できる状態を実現する。Loki 自身は Prometheus のスクレイプ対象として登録し Constitution 原則 V を満たす。
...
**Target Platform**: リモートDocker Engine（YOUR_SERVER_IP / Ubuntu）
