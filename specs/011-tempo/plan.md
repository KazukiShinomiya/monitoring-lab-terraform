...
**Target Platform**: リモート Docker Engine (YOUR_SERVER_IP), WSL2 経由の Terragrunt
...
#### A7: 設定ファイル転送 + コンテナ確認
```bash
wsl ... ssh ubuntu@YOUR_SERVER_IP 'mkdir -p ~/monitoring-lab/tempo ~/monitoring-lab/otel-collector'
# scp 設定ファイル → remote
...
#### D1: Tempo ヘルスチェック
```bash
curl -s http://YOUR_SERVER_IP:3200/ready  # "ready" を確認
```
...
- http://YOUR_SERVER_IP:9090/targets → `tempo` ジョブが UP
