...
Claude CodeがリモートDocker（YOUR_SERVER_IP）をSSH経由で操作できるMCPサーバーをTypeScriptで実装する。
...
**Docker接続**: `child_process` 経由で `docker -H ssh://ubuntu@YOUR_SERVER_IP` CLI を実行
...
**Constraints**: SSH鍵を `/root/.ssh/id_rsa` にマウント、リモートホスト `YOUR_SERVER_IP` のみ対象

## Constitution Check

### I. Infrastructure as Code (IaC) ✅

本MCPサーバーはリモート監視基盤（YOUR_SERVER_IP）ではなく、ローカル開発環境上で動作する開発ツール。
