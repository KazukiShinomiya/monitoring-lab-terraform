...
**Purpose**: Self-hosted Runner コンテナを 10.0.0.220 にデプロイし GitHub にオンライン登録する。US1/US2/US3 すべての前提条件。
...
- [x] T009 [US4] `terragrunt apply` を実行してリモートサーバー (10.0.0.220) にコンテナをデプロイ
- [x] T010 [US4] 動作確認: `wsl -d Ubuntu-24.04 -e bash -c "ssh ubuntu@10.0.0.220 'docker ps | grep github-runner'"` で `monitoring-lab-github-runner` が Up を確認
