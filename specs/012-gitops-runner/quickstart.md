...
```bash
# Runner コンテナを手動停止
wsl -d Ubuntu-24.04 -e bash -c "ssh ubuntu@YOUR_SERVER_IP 'docker stop monitoring-lab-github-runner'"

# 5分以内に自動再起動することを確認
wsl -d Ubuntu-24.04 -e bash -c "ssh ubuntu@YOUR_SERVER_IP 'docker ps | grep github-runner'"
```
...
```bash
# コンテナログを確認
wsl -d Ubuntu-24.04 -e bash -c "ssh ubuntu@YOUR_SERVER_IP 'docker logs monitoring-lab-github-runner'"
