...
Add the following job to `prometheus.yml` on the Prometheus server (YOUR_SERVER_IP):
...
scrape_configs:
  ...
  - job_name: "windows_exporter"
    ...
    static_configs:
      - targets:
          - "YOUR_WINDOWS_HOST_1:9182"    # Example: Windows workstation
          - "YOUR_WINDOWS_HOST_2:9182"    # Example: Windows server 1
          - "YOUR_WINDOWS_HOST_3:9182"    # Example: Windows server 2
        labels:
          env: "local"
```
...
[
  {
    "targets": ["YOUR_WINDOWS_HOST_1:9182", "YOUR_WINDOWS_HOST_2:9182"],
    "labels": {
      "env": "local",
      "os": "windows"
    }
...
```bash
# Test connectivity from Prometheus host (YOUR_SERVER_IP)
curl -s http://<WINDOWS_HOST_IP>:9182/metrics | head -20

# Check target status in Prometheus
curl -s http://YOUR_SERVER_IP:9090/api/v1/targets | jq '.data.activeTargets[] | select(.labels.job=="windows_exporter")'
```
