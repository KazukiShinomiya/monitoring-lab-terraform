...
- `~/.ssh/id_rsa` が WSL2 のホームディレクトリに存在すること（リモートサーバー YOUR_SERVER_IP への接続用）
...
**接続先**: `http://YOUR_SERVER_IP:9090`（環境変数 `PROMETHEUS_URL` で変更可）
...
**接続先**: `ssh://ubuntu@YOUR_SERVER_IP`（Docker over SSH）
...
**接続先**: `http://YOUR_SERVER_IP:9093`（環境変数 `ALERTMANAGER_HOST` で変更可）
