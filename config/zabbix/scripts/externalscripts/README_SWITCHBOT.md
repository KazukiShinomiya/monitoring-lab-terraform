...
1. Zabbix Web UI (http://YOUR_SERVER_IP:8080) にログイン
...
# コンテナ再起動
ssh ubuntu@YOUR_SERVER_IP "docker restart monitoring-lab-zbx_server"
```
...
1. スクリプトの実行権限
   ```bash
   ssh ubuntu@YOUR_SERVER_IP "ls -la ~/monitoring-lab/zabbix/scripts/externalscripts/"
   ```
