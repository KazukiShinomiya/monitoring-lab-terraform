...
| メモリ | 256MB |
| LAN | 2ポート |
| LAN1 IP | YOUR_ROUTER_IP/24（自宅LAN側） |
...
```
【監視サーバー（YOUR_SERVER_IP）】          【RTX830（YOUR_ROUTER_IP）】
         │                                      │
         │  「sysDescrを教えて」（ポーリング）  │
         │ ─────────────────────────────────> │
         │                                      │
...
### `snmp host YOUR_SERVER_IP community monlab version 2`

```
snmp        → SNMPの設定コマンド
host        → 接続先ホストを指定
YOUR_SERVER_IP  → 監視サーバーのIP
...
### `snmpwalk -v2c -c monlab YOUR_ROUTER_IP sysDescr.0`
...
→ YOUR_SERVER_IP → YOUR_ROUTER_IP の SNMP通信は通る ✅
```
...
| 問い合わせ元IPを限定 | YOUR_SERVER_IP以外は不可 | 低（Yamaha対応要確認） |
