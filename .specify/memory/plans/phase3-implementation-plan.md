...
- Docker Engine (リモートホスト YOUR_SERVER_IP)
...
```bash
# cAdvisor稼働確認
curl http://YOUR_SERVER_IP:8080/metrics | grep container_cpu

# Prometheusからスクレイプ確認
curl http://YOUR_SERVER_IP:9090/api/v1/targets | jq '.data.activeTargets[] | select(.job=="cadvisor")'
```
...
# メトリクス取得確認
curl http://YOUR_SERVER_IP:9090/api/v1/query?query=container_memory_usage_bytes
```
...
```bash
# アラートルール確認
curl http://YOUR_SERVER_IP:9090/api/v1/rules

# 手動でコンテナ停止してアラート発火テスト
ssh ubuntu@YOUR_SERVER_IP "docker stop monitoring-lab-vault"
# 1分後にアラート確認
...
**出力**:
- Web UI (`http://YOUR_SERVER_IP:3000/d/monitoring-lab-overview`)
...
```bash
# ダッシュボード存在確認
curl -u admin:admin http://YOUR_SERVER_IP:3000/api/dashboards/uid/monitoring-lab-overview

# データソース接続確認
curl -u admin:admin http://YOUR_SERVER_IP:3000/api/datasources
```
...
5. `terragrunt apply`
6. cAdvisor稼働確認: `curl http://YOUR_SERVER_IP:8080/metrics`
...
2. リモートホストにファイルアップロード:
   ```bash
   scp config/prometheus/prometheus.yml ubuntu@YOUR_SERVER_IP:~/monitoring-lab/prometheus/
   ```
3. Prometheus設定リロード:
   ```bash
   curl -X POST http://YOUR_SERVER_IP:9090/-/reload
   ```
4. ターゲット確認: `http://YOUR_SERVER_IP:9090/targets`
...
3. リモートホストにファイルアップロード:
   ```bash
   scp config/prometheus/alerts.yml ubuntu@YOUR_SERVER_IP:~/monitoring-lab/prometheus/
   ```
4. `terraform/envs/local/prometheus/terragrunt.hcl`更新（Bind Mount追加）
5. `terragrunt plan && terragrunt apply` (Prometheusコンテナ再作成)
6. アラート確認: `http://YOUR_SERVER_IP:9090/alerts`

#### ステップ4: Grafanaダッシュボード

1. Grafana UIで手動作成 (`http://YOUR_SERVER_IP:3000`)
2. JSONエクスポート
...
5. リモートホストにファイルアップロード:
   ```bash
   scp -r config/grafana/provisioning/dashboards/ ubuntu@YOUR_SERVER_IP:~/monitoring-lab/grafana/provisioning/
   ```
6. Grafana再起動:
   ```bash
   ssh ubuntu@YOUR_SERVER_IP "docker restart monitoring-lab-grafana"
   ```
7. ダッシュボード確認: `http://YOUR_SERVER_IP:3000/d/monitoring-lab-overview`

#### ステップ5: 全体検証

1. すべてのターゲットが"UP"状態: `http://YOUR_SERVER_IP:9090/targets`
2. アラートルールが読み込まれている: `http://YOUR_SERVER_IP:9090/rules`
3. Grafanaダッシュボードが表示される: `http://YOUR_SERVER_IP:3000`
...
# Prometheus再起動
ssh ubuntu@YOUR_SERVER_IP "docker restart monitoring-lab-prometheus"
```
...
# ダッシュボード削除（UI経由）
# または、プロビジョニングファイル削除
ssh ubuntu@YOUR_SERVER_IP "rm -rf ~/monitoring-lab/grafana/provisioning/dashboards/*"
ssh ubuntu@YOUR_SERVER_IP "docker restart monitoring-lab-grafana"
```
...
- [ ] **SC-001**: Prometheusが9つのターゲットからメトリクス収集
  - `curl http://YOUR_SERVER_IP:9090/api/v1/targets | jq '.data.activeTargets | length'` → `2` (prometheus + cadvisor)

- [ ] **SC-002**: Grafanaダッシュボードが3秒以内に表示
  - ブラウザで `http://YOUR_SERVER_IP:3000/d/monitoring-lab-overview` アクセス
...
```bash
# Prometheusターゲット確認
curl http://YOUR_SERVER_IP:9090/api/v1/targets | jq

# アラート確認
curl http://YOUR_SERVER_IP:9090/api/v1/alerts | jq

# Grafanaダッシュボード一覧
curl -u admin:admin http://YOUR_SERVER_IP:3000/api/search | jq

# cAdvisorメトリクス確認
curl http://YOUR_SERVER_IP:8080/metrics | grep container_cpu

# リモートホストのファイル確認
ssh ubuntu@YOUR_SERVER_IP "ls -la ~/monitoring-lab/prometheus/"
ssh ubuntu@YOUR_SERVER_IP "cat ~/monitoring-lab/prometheus/prometheus.yml"
```
