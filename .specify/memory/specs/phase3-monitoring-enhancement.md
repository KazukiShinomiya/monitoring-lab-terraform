...
**Independent Test**:
Grafana (`http://YOUR_SERVER_IP:3000`) にアクセスし、「Monitoring Lab Overview」ダッシュボードが表示される。
...
- **FR-003**: スクレイプターゲットはリモートDockerホスト（YOUR_SERVER_IP）のDocker Engineメトリクスエンドポイント
