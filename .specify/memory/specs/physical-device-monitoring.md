...
**Independent Test**:
`http://10.0.0.220:9090/targets` でSNMPターゲット（RTX830）が `UP` 状態であることを確認。
...
**Independent Test**:
`http://10.0.0.220:9090/targets` でSNMPターゲット（Synology）が `UP` 状態であることを確認。
...
| コンポーネント | 種別 | 管理方法 | 配置先 |
|--------------|------|---------|--------|
| SNMP Exporter | 新規コンテナ | Terragrunt | 10.0.0.220 |
...
snmp host 10.0.0.220 community public version 2
save
```
...
ufw allow from 10.0.0.220 to any port 9100
```
...
New-NetFirewallRule -DisplayName "Windows Exporter" `
  ...
  -RemoteAddress 10.0.0.220 -Action Allow
```
...
- Node Exporter / Windows Exporter は監視サーバー（10.0.0.220）からのみアクセス許可
