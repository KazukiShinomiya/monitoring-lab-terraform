...
- [ ] T005 cAdvisorの稼働確認: `curl http://10.0.0.220:8080/metrics | grep container_cpu`
...
- [ ] T006 リモートホストのディレクトリ構造を確認: `ssh ubuntu@10.0.0.220 "ls -la ~/monitoring-lab/prometheus/"`
- [ ] T007 Grafanaプロビジョニングディレクトリを確認: `ssh ubuntu@10.0.0.220 "ls -la ~/monitoring-lab/grafana/provisioning/"`
...
**独立テスト**:
- `http://10.0.0.220:9090/targets` ですべてのターゲットが "UP" 状態
...
- [ ] T009 [US1] prometheus.ymlをリモートホストにアップロード: `scp config/prometheus/prometheus.yml ubuntu@10.0.0.220:~/monitoring-lab/prometheus/`
- [ ] T010 [US1] Prometheus設定をリロード: `curl -X POST http://10.0.0.220:9090/-/reload`
- [ ] T011 [US1] スクレイプターゲットを確認 `http://10.0.0.220:9090/targets` - cAdvisorが"UP"であること
- [ ] T012 [US1] コンテナメトリクスを確認: `curl "http://10.0.0.220:9090/api/v1/query?query=container_cpu_usage_seconds_total" | jq`
...
**独立テスト**:
- `http://10.0.0.220:3000` で「Monitoring Lab Overview」ダッシュボードが表示される
...
- [ ] T015 [US2] dashboardsディレクトリをリモートホストにアップロード: `scp -r config/grafana/provisioning/dashboards/ ubuntu@10.0.0.220:~/monitoring-lab/grafana/provisioning/`
- [ ] T016 [US2] Grafanaを再起動してプロビジョニングされたダッシュボードをロード: `ssh ubuntu@10.0.0.220 "docker restart monitoring-lab-grafana"`
- [ ] T017 [US2] ダッシュボードを確認 `http://10.0.0.220:3000/dashboards` - 「Monitoring Lab Overview」が存在すること
...
**独立テスト**:
- `http://10.0.0.220:9090/alerts` でアラートルールが表示される
...
- [ ] T021 [US3] alerts.ymlをリモートホストにアップロード: `scp config/prometheus/alerts.yml ubuntu@10.0.0.220:~/monitoring-lab/prometheus/`
- [ ] T022 [US3] 更新したprometheus.ymlをアップロード: `scp config/prometheus/prometheus.yml ubuntu@10.0.0.220:~/monitoring-lab/prometheus/`
...
- [ ] T025 [US3] アラートルールのロードを確認 `http://10.0.0.220:9090/rules`
- [ ] T026 [US3] コンテナを停止してアラートをテスト: `ssh ubuntu@10.0.0.220 "docker stop monitoring-lab-vault"`
- [ ] T027 [US3] 1分後にアラートが発火していることを確認 `http://10.0.0.220:9090/alerts`
- [ ] T028 [US3] テストコンテナを再起動: `ssh ubuntu@10.0.0.220 "docker start monitoring-lab-vault"`
...
- [ ] T029 [US4] GrafanaでZabbixデータソースが設定されているか確認 `http://10.0.0.220:3000/datasources`
- [ ] T030 [US4] 統合ダッシュボードJSONを作成 `config/grafana/provisioning/dashboards/integrated-monitoring.json`
- [ ] T031 [US4] 統合ダッシュボードをアップロード: `scp config/grafana/provisioning/dashboards/integrated-monitoring.json ubuntu@10.0.0.220:~/monitoring-lab/grafana/provisioning/dashboards/`
- [ ] T032 [US4] Grafanaを再起動: `ssh ubuntu@10.0.0.220 "docker restart monitoring-lab-grafana"`
